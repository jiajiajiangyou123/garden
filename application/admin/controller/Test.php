...
class Test extends Controller
{
    ...
    {
        echo openssl_cipher_iv_length('md5');exit;
        ob_end_clean();//关闭缓存
        set_time_limit(0);
    ...
    public function excel()
    {
        ...
        $s = 1992004;$e = 1993004;
        $res = db('test')->order('id desc')->find();
        $id = $res['id'];

        while ($id > $s) {
            $data = db('test')->where('id','between',[$s,$e])->select();
            $s += 1000; $e += 1000;
            if ($data) {
                $data = collection($data)->toArray();
                foreach ($data as $i => $v) {
                    foreach ($v as $k=>$value) {
                        $v[$k] = iconv('utf-8', 'gb2312', $value);
                    }
                    fputcsv($fp, $v);
                }
            }
    ...
    public function accesslog()
    {
        ...
        $file = 'file:///F:/www/php/Garden/public/static/web/css/access.log';
        ...
        {
            $data[]['line'] = $string;//获取当前行的数据
            $count = count($data);
            if ($count == 200) {
                db('line')->insertAll($data);
                $data = [];
            }
        }
        fclose($fp);
